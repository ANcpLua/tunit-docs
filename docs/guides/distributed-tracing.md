...
The general setup in [OpenTelemetry Tracing](/docs/examples/opentelemetry.md#setup) works everywhere. Backend-specific notes follow.

Zero-config setup

Install [`TUnit.OpenTelemetry`](/docs/examples/opentelemetry.md#option-a-zero-config-tunitopentelemetry) and set `OTEL_EXPORTER_OTLP_ENDPOINT`. The package auto-wires a `TracerProvider` at test discovery, pre-registers `TUnitTestCorrelationProcessor`, and flushes at session end. Call `TUnitOpenTelemetry.Configure(...)` to add exporters or processors.
...
Aspire is wired up automatically through `TUnit.Aspire`. See [Aspire integration](/docs/examples/aspire.md). The dashboard understands the link between `test case` and `test suite` spans, so it groups them naturally.
...
* Add the [`TUnitTagProcessor`](/docs/examples/opentelemetry.md#spans-from-test-a-are-showing-up-under-test-b) so spans always carry `tunit.test.id` even when the trace ID is wrong, then filter by tag.
...
Use [`TestWebApplicationFactory<T>`](/docs/examples/aspnet.md) or wrap with `TracedWebApplicationFactory<T>`. The `TUnit0064` analyzer raises a warning (with a code fix) when a class inherits directly from `WebApplicationFactory<T>`.
...
Install [`TUnit.OpenTelemetry`](/docs/examples/opentelemetry.md#option-a-zero-config-tunitopentelemetry) and an OTLP/HTTP receiver starts automatically at test discovery. Spawned child processes, testcontainers, or any SUT reachable from the test host can push spans into TUnit's HTML report by exporting OTLP to that receiver.
...
* [OpenTelemetry Tracing](/docs/examples/opentelemetry.md) — first-time setup, full attribute reference, troubleshooting.
* [ASP.NET Core Integration Testing](/docs/examples/aspnet.md) — `TestWebApplicationFactory<T>` setup.
* [HTML Test Report — Distributed Tracing](/docs/guides/html-report.md#distributed-tracing) — how the HTML report renders the data.
* [Aspire integration](/docs/examples/aspire.md) — Aspire dashboard and OTLP receiver.
