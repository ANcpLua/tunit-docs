...
For dependency injection with service providers, inherit from `DependencyInjectionDataSourceAttribute<TScope>` to create custom attributes. See [Dependency Injection](/docs/writing-tests/dependency-injection.md) documentation for details.
...
See [Test Lifecycle — Initialization Interfaces](/docs/writing-tests/lifecycle.md#initialization-interfaces) for the full timing diagram.
...
For more troubleshooting, see [Tests Not Discovered](/docs/troubleshooting.md#tests-not-discovered).
...
When using nested property injection, the `Shared` parameter controls object lifetime and reuse. Use `SharedType.PerTestSession` for expensive resources like containers. See [ClassDataSource](/docs/writing-tests/class-data-source.md) for all options.
